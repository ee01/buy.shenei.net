...
class ModuleObject extends MasterObject
{
	...
	function DoSend()
	{
		...
		$onlineip=$_SERVER['REMOTE_ADDR'];
				$email_message="{$member['username']}，您好：
这封信是由 {$this->Config['site_name']} 发送的。
您收到这封邮件，是因为在{$this->Config['site_name']}上这个邮箱地址被登记为用户邮箱，
且该用户请求使用 Email 密码重置功能所致。
----------------------------------------------------------------------
重要！
----------------------------------------------------------------------
如果您没有提交密码重置的请求或不是{$this->Config['site_name']}的注册用户，请立即忽略
...
{$this->Config['site_url']}";
	...
	function DoReset()
	{
		...
		$this->DatabaseHandler->Query($sql);
		$this->Messager("新密码设置成功,现在为您转入登陆界面.","?mod=login");
	}
