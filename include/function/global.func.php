...
function dfopen($url, $limit = 10485760 , $post = '', $cookie = '', $bysocket = false,$timeout=5,$agent="") {
	...
	$matches = parse_url(unescape($url));
	$host = $matches['host'];
...
/*Modify By 01*/
function sendmail($uname,$emailaddress,$title,$content){
	require("./setting/product.php");
	if($config['product']['default_mailtype']==1){
		stmp_mail($uname,$emailaddress,$title,$content);
	}else{
		mail_mail($uname,$emailaddress,$title,$content);
	};
}
...
/*Modify By 01*/
function stmp_mail($uname,$emailaddress,$title,$content){
//	require_once(MOD_PATH.'mail/class.phpmailer.php');
	require_once(ROOT_PATH.'modules/admin/mail/class.phpmailer.php');
	require(CONFIG_PATH.'product.php');  
	$mail = new PHPMailer(); 
	$mail->CharSet ="GB2312"; 
	$mail->IsSMTP();  
	$mail->Host = $config['product']['default_server'];  
	$mail->SMTPAuth = true;  
	$mail->Username = $config['product']['default_user'];  
	$mail->Password = $config['product']['default_pwd']; 
	require(CONFIG_PATH.'settings.php');  
	$mail->From = $config['site_admin_email']; 
	$mail->FromName = $config['site_name']; 
	$mail->AddAddress($emailaddress,$uname); 
	$mail->WordWrap = 50;  $mail->IsHTML(true);  $mail->Subject = $title; 
	$mail->Body = $content; 
	if(!$mail->Send()){ 
...
/*Modify By 01*/
function mail_mail($uname,$emailaddress,$title,$content){
	require(CONFIG_PATH.'settings.php'); 
