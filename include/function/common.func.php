...
function position()
{
	...
	$list['index']="<a href='{$config['site_url']}'>{$config['site_name']}(Ê×Ò³)</a>";
...
function error($type, $message, $file = null, $line = 0) { if(E_NOTICE==$type)return true; require_once(LIB_PATH."error.han.php"); if(false == class_exists("ErrorHandler"))return false; $ErrorHandler = new ErrorHandler($type, $message, $file, $line); if(error_reporting() && $type)exit($ErrorHandler->fatal()); return true; }
