...
function instheader() {
	...
	echo "<html><head>".
		...
		"<title>TTtuangou Installation Wizard $version </title>".
		...
		"<table width=\"95%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" bgcolor=\"#FFFFFF\" align=\"center\"><tr><td>".
          	...
          	"<td class=\"install\" height=\"30\" valign=\"bottom\"><font color=\"#FF0000\">&gt;&gt;</font> TTtuangou $version&nbsp;$lang[install_wizard] ".
...
function instfooter() {
	...
	echo "<tr><td><hr noshade align=\"center\" width=\"100%\" size=\"1\"></td></tr>".
        	"<tr><td align=\"center\">".
            	"<b style=\"font-size: 11px\">Powered by <a href=\"http:/"."/TTtuangou.net\" target=\"_blank\">TTtuangou $version".
          	"</a> &nbsp; Copyright &copy; <a href='http:/"."/www.cenwor.com' target='_blank'>Cenwor Inc.</a> 2005 - 2010</b><br /><br />".
...
function runquery($sql) {
	...
	$sql = str_replace("\r", "\n", str_replace('`myth_',"`" . $config['db_table_prefix'], $sql));
	$ret = array();
	$num = 0;
	foreach(explode(";\n", trim($sql)) as $query) {
		$queries = explode("\n", trim($query));
	...
	foreach($ret as $query) {
		...
		if($query) {
			if(substr($query, 0, 13) == 'CREATE TABLE ') {
				...
				$_sql = createtable($query, $dbcharset);
				$db->query($_sql);
